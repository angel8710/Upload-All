
<!DOCTYPE html>

<!--[if lt IE 7 ]> <html class="ie6"> <![endif]-->
<!--[if IE 7 ]>    <html class="ie7"> <![endif]-->
<!--[if IE 8 ]>    <html class="ie8"> <![endif]-->
<!--[if IE 9 ]>    <html class="ie9"> <![endif]-->
<!--[if (gt IE 9)|!(IE)]><!--> <html lang="en"> <!--<![endif]-->

    <head>

        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="author" content="BenchMark Finance">

        <link rel="shortcut icon" href="https://www.benchmark.bg/academy/favicon.ico">
       

        <title>Стани успешен финансов трейдър с БенчМарк Академия</title>
        <meta name="title" content="">
        <meta name="description" content="" />
        <meta name="keywords" content="" />

        <!--[if gte IE 9]>
          <style type="text/css">
            .gradient {
               filter: none;
            }
          </style>
        <![endif]-->

        <!-- Bootstrap Core CSS -->
        <link href="css/bootstrap.min.css" rel="stylesheet">

        <!-- Custom CSS -->
        <link href="css/custom.css" rel="stylesheet">

        <link href="css/custom-t.css" rel="stylesheet">
        <link href="css/custom-p.css" rel="stylesheet">

        <!-- Custom Fonts -->
        <link href="css/font-awesome.min.css" rel="stylesheet" type="text/css">

        <link href='http://fonts.googleapis.com/css?family=Open+Sans:400,700&subset=latin,cyrillic' rel='stylesheet' type='text/css'>
        <link href='https://fonts.googleapis.com/css?family=Open+Sans+Condensed:300,700&subset=latin,cyrillic-ext' rel='stylesheet' type='text/css'>

        <!--[if lt IE 9]>
            <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
            <script src="https://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
        <![endif]-->

    </head>

    <body>
        <!-- Navigation -->
        <!-- Secondary Menu -->

        <nav class="navbar navbar-inverse">
            <div class="container">
                <div class="navbar-header" id="navbar" style="margin-top: -28px; padding-top: 15px; height: 89px;">               
                    <a class="navbar-brand" href="http://www.benchmark.bg/landing/albena/">
                        <img src="https://www.benchmark.bg/academy/wp-content/themes/benchmark_academy/images/bm-academy-logo.svg" width="190" class="center-block unveil-loaded down" style="margin-top: -7px; margin-left:-7px;"></a>
                    <span class="mobile-menu hidden-xs hidden-md hidden-sm hidden-lg">
                        <ul class="nolist">
                            <li><a target="_blank" href="/За-нас_456.html">За нас</a></li> |
                            <li><a target="_blank" href="https://www.benchmark.bg/academy/%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%D0%B8/">Контакти</a></li>
                        </ul>
                    </span>
                </div>           
                <div id="">
                    <ul class="nav navbar-nav navbar-right secondary-menu hidden-xs" style="width: 520px; position: relative;">
                        <li><a href="https://benchmark.bg/academy/" target="_blank"><span style="color:#002662;">Сайт за обучение&nbsp;<i class="fa fa-graduation-cap" aria-hidden="true"></i></span></a></li>   
                        <li><a href="http://benchmark.bg" target="_blank">Сайт за търговия <span class="fa fa-sign-in"></span></a></li>				
                        <li><a href="https://www.benchmark.bg/academy/%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%D0%B8/" target="_blank">Контакти <i class="fa fa-phone" aria-hidden="true"></i><span style="color:#7b0f17;"></span></a></li>
                        <li style="margin-top: 9px;">
                            <div id="comm100-button-541" style="margin-right: -10px;"><a href="#" onclick="Comm100API.open_chat_window(event, 541);" 
                                                                                         style="color: #002662 !important;">Live chat</a></div>
                            <script async="" src="https://www.google-analytics.com/analytics.js"></script><script type="text/javascript" async="" defer="" src="https://www.benchmark.bg/piwik/piwik.js"></script><script src="https://connect.facebook.net/signals/config/1055135051179489?v=2.7.7" async=""></script><script async="" src="https://connect.facebook.net/en_US/fbevents.js"></script><script type="text/javascript" async="" src="https://chatserver.comm100.com/livechat.ashx?siteId=199228"></script><script type="text/javascript" async="" src="https://chatserver.comm100.com/livechat.ashx?siteId=199228"></script><script type="text/javascript">
                                var Comm100API = Comm100API || {chat_buttons: []};
                                Comm100API.chat_buttons.push({code_plan: 541, div_id: 'comm100-button-541'});
                                Comm100API.site_id = 199228;
                                Comm100API.main_code_plan = 541;
                                (function () {
                                    var lc = document.createElement('script');
                                    lc.type = 'text/javascript';
                                    lc.async = true;
                                    lc.src = 'https://chatserver.comm100.com/livechat.ashx?siteId=' + Comm100API.site_id;
                                    var s = document.getElementsByTagName('script')[0];
                                    s.parentNode.insertBefore(lc, s);
                                })();
                            </script></li>
                        <li style="margin-top: 8px;"><span class="fa fa-comments brand-color"></span></li>
                    </ul>
                </div>           
            </div>
            <!-- /.container -->
        </nav>

        <header>
            <div class="wrap">
                <div class="container">
                    <div class="row text-left">
                        <div class="col-xs-12">
                            <h1 class="topHeading">Първи стъпки в света на финансовите пазари</h1>
                        </div>
                    </div>
                    <div class="row firstRow">
                        <div class="col-xs-12 col-sm-12 col-md-7 col-lg-9">

                            <h2 class="topHeading2 secondHeading">Включете се в състезание на финансовите пазари с награден фонд <b style="font-size: 40px;" class="twoThousand">2000</b> лв.</h2><br/>
                            <div class="info">
                                <p><span style="color:#c72027;font-weight: bold;margin-top:40px;">Начало на състезанието:</span> 00:00 ч. на 28.08.2017 г.</p>
                                <p><span style="color:#c72027;font-weight: bold;margin-top:40px;">Край на състезанието:</span> 24:00 ч. на 01.09.2017 г.</p>
                                <p><span style="color:#c72027;font-weight: bold;margin-top:40px;">Класиране:</span> 04.09.2017 г.</p>
                            </div>
                  
                            <h2 class="topHeading2 firstHeading">Регистрирайте се за безплатно обучение
                            </h2>
                            <br />
                            <h2 class="topHeading2 secondHeading">Включете се в състезание на финансовите пазари с награден фонд <b style="font-size: 40px;" class="twoThousand">2000</b> лв.</h2><br/>
                            <div class="info">
                                <p><span style="color:#c72027;font-weight: bold;margin-top:40px;">Къде:</span> к.к. Албена</p>
                                <p><span style="color:#c72027;font-weight: bold;">Кога:</span> 27 август 2017 г.</p>
                                <p><span style="color:#c72027;font-weight: bold;">Начален час:</span> 14:00 ч.</p>
                                <p><span style="color:#c72027;font-weight: bold;">Продължителност:</span> 2 часа</p>
                            </div>
            

                            <div class="row">

                            <div class="hidden-xs hidden-sm col-md-12">
                                <a href="http://www.benchmark.bg/landing/albena/ranking.php">
                                    <button class="pro-btn">Крайно класиране &nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button></a>
                            </div>
                            <div class="hidden-xs hidden-sm col-md-12">
                                <a href="http://www.benchmark.bg/landing/albena/survey.php">
                                    <button class="pro-btn">Анкетна карта &nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button></a>
                            </div>
                            <div class="hidden-xs hidden-sm col-md-12">
                                    <button class="pro-btn pro-btn-bottom conditionsButton" type="submit" onclick="window.open('img/Условия на състезанието.pdf')">Условия на състезанието &nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button>
                            </div>
            
                                <div class="hidden-xs hidden-sm col-md-12">
                                    <!--<button type="submit" onclick="window.open('file.doc')">Download!</button>-->
                                    <button class="pro-btn" type="submit" onclick="window.open('img/Програма_27.08.pdf')">Програма на обучението&nbsp;&nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button>
                                </div>
                                <div class="hidden-xs hidden-sm col-md-12">
                                    <button class="pro-btn pro-btn-bottom" type="submit" onclick="window.open('img/Условия на състезанието.pdf')">Условия на състезанието &nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button>
                                </div>

                            </div>
                        </div>


                        <div class="col-xs-12 col-sm-12 col-md-5 col-lg-3" id="demoForm" style="display:block;">		
                            <div class="formWrapper" style="background: rgba(35, 30, 30, .7);">					
                                <div class="panel-demo metatrader demo-active phheight">
                                    <div class="panel-heading panel-heading-demo padding_top_bottom_20">
                                        <span class="real real-m specialBgSiteFontSize" style=" color: #ffffff;line-height: 1.1em;font-weight: bold;">Валути,&nbsp;акции, метали, стоки, индекси</span>
                                        <div class="row">
                                            <div class="col-xs-12" style="margin-top: -5px;">
                                                <img src="img/linedivider.png" class="img-responsive center-block linedivider">
                                            </div>	
                                        </div>
                                        <h4 class="real-mm" style="color: #ffffff;line-height: 1.1em;font-weight: bold;font-size: 20px;">MetaTrader</h4>
                                    </div>
                                    <div class="panel-body platforms-body first">
                                        <div style="padding-top:19px;" class="panel-image text-center"><img alt="" src="./img/demo-monitor1.png">
                                        </div>
                                        <br>
                                        <p style="font-size:18px;" id="text-demo" class="blue-color"><strong>Безплатна демо сметка <br> $100 000</strong></p>
                                    </div>						
                                </div>
                                <div>
                                    <button type="submit" class="formHeadingButton" onclick="document.location.href = '/academy/начало/практика/безплатна-демо-сметка/'"><span class="small"  style="display: block;margin-top:-2px !important">Започнете сега</span></button>
                                </div>					
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-12 col-md-5 col-lg-3" id="regForm" style="display:none;">		
                            <div class="formWrapper">
                                <form method="get" name="frm_registration" id="frm_registration" action="">
                                    <input type="hidden" name="obj_id" id="obj_id" value="">
                                    <div class="formHeading">
                                        <p class="small">РЕГИСТРАЦИЯ ЗА УЧАСТИЕ</p>
                                    </div>
                                    <div class="formName">
                                        <input type="text" name="fln" id="fln" placeholder="Име и фамилия" onFocus="jQuery(this).attr('placeholder', '');" onBlur="jQuery(this).attr('placeholder', 'Име и фамилия');" />
                                    </div>
                                    <div class="formMail">
                                        <input type="text" name="email" id="email" placeholder="Имейл адрес" onFocus="jQuery(this).attr('placeholder', '');" onBlur="jQuery(this).attr('placeholder', 'Имейл адрес');" />
                                    </div>
                                    <div class="formTel">
                                        <input type="text" name="phone" id="phone" placeholder="Телефон" onFocus="jQuery(this).attr('placeholder', '');" onBlur="jQuery(this).attr('placeholder', 'Телефон');"/>
                                    </div>
                                    <div class="formSpecialty">
                                        <input type="text" name="specialty" id="specialty" placeholder="Университет" onFocus="jQuery(this).attr('placeholder', '');" onBlur="jQuery(this).attr('placeholder', 'Университет');" />
                                    </div>		
                                    <div>
                                        <button type="submit" class="formHeadingButton"><span class="small"  style="display: block;margin-top:-2px !important">Регистрирайте се</span></button>
                                    </div>
                                </form>
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-12 hidden-md hidden-lg">
                            <a href="http://www.benchmark.bg/landing/albena/ranking.php">
                                <button class="pro-btn">Крайно класиране &nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button></a>
                        </div>
                        <div class="col-xs-12 col-sm-12 hidden-md hidden-lg">
                            <a href="http://www.benchmark.bg/landing/albena/survey.php">
                                <button class="pro-btn">Анкетна карта&nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button></a>
                        </div>
                        <div class="col-xs-12 col-sm-12 hidden-md hidden-lg">
                                <button class="pro-btn pro-btn-bottom conditionsButton" type="submit" onclick="window.open('img/Условия на състезанието.pdf')">Условия на състезанието &nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button>
                        </div>
                  
                        <div class="col-xs-12 col-sm-12 hidden-md hidden-lg">
                            <!--<button type="submit" onclick="window.open('file.doc')">Download!</button>-->
                            <button class="pro-btn" type="submit" onclick="window.open('img/Програма_27.08.pdf')">Програма на обучението&nbsp;&nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button>
                        </div>
                        <div class="col-xs-12 col-sm-12 hidden-md hidden-lg">
                            <button class="pro-btn pro-btn-bottom" type="submit" onclick="window.open('img/Условия на състезанието.pdf')">Условия на състезанието &nbsp;<i class="fa fa-angle-right" style="font-size:20px"></i></button>
                        </div>

                     </div>
                </div>
        </header>

        <script type="text/javascript" src="/js/jquery-1.11.1.min.js"></script>	
        <script type="text/javascript" src="/js/bootstrap.min.js"></script>	
        <div class="modal fade" tabindex="-1" role="dialog" id="myModal">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Затвори"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title">Информация</h4>
                    </div>
                    <div class="modal-body">

                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-demo-mobile btnModalClose" data-dismiss="modal">Затвори</button>
                    </div>
                </div>
                <!-- /.modal-content --> 
            </div>
            <!-- /.modal-dialog --> 
        </div>
        <!-- /.modal -->  

        <script src="/js/jquery.validate.min.js"></script> 
        <script type="text/javascript">
            jQuery(document).ready(function (e) {
                var validator = jQuery("#frm_registration").validate({
                    onfocusout: false,
                    onkeyup: false,
                    onclick: false,
                    rules: {
                        fln: {required: true, minlength: 6},
                        email: {required: true, email: true},
                        phone: {required: true, minlength: 2, maxlength: 40},
                        specialty: {required: true},
                        obj_id: {required: true},
                        form_sid: {required: true}
                    },
                    submitHandler: function (form) {
                        jQuery('#fwd').attr('value', 'Моля, изчакайте...').attr('disabled', 'disabled');
                        jQuery.ajax({
                            url: "../../register_seminar.php",
                            data: {
                                email: jQuery("#email").val(),
                                phone: jQuery("#phone").val(),
                                fln: jQuery("#fln").val() + " (" + jQuery("#specialty").val() + ")",
                                form_sid: "<?php echo isset($_SESSION['form_sid']) ? $_SESSION['form_sid'] : ""; ?>",
                                ref: "landing",
                                id: jQuery("#obj_id").val(),
                                seminar_type: "university",
                                campain_ind: "<?php echo isset($_SESSION['campain_ind']) ? $_SESSION['campain_ind'] : "" ?>"
                            },
                            type: "GET",
                            cache: false,
                            dataType: "html",
                            async: false
                        })
                                .done(function (data, textStatus, jqXHR) {
                                    jQuery('#fwd').attr('value', 'Регистрирай се').removeAttr('disabled');
                                    document.getElementById("frm_registration").reset();
                                    jQuery('#myModal .modal-title').html("Регистрация");
                                    jQuery('#myModal .modal-body').html(data);
                                    jQuery('#myModal').modal({});
                                })
                                .fail(function (jqXHR, textStatus, errorThrown) {
                                    jQuery('#fwd').attr('value', 'Регистрирай се').removeAttr('disabled');
                                    document.getElementById("frm_registration").reset();
                                    jQuery('#myModal .modal-title').html("Грешка");
                                    jQuery('#myModal .modal-body').html(textStatus + "<br>" + errorThrown);
                                    jQuery('#myModal').modal({});
                                });
                    },
                    messages: {
                        fln: {
                            required: "Полето е задължително.",
                            minlength: "Полето трябва да съдържа поне 6 символа."
                        },
                        email: {
                            required: "Полето е задължително.",
                            email: "Въвели сте невалиден имейл."
                        },
                        phone: {
                            required: "Полето е задължително.",
                            minlength: "Полето трябва да съдържа поне 2 цифри.",
                            maxlength: "Полето трябва да съдържа до 40 цифри."
                        },
                        specialty: {
                            required: "Полето е задължително."
                        },
                        obj_id: {
                            required: "Полето е задължително."
                        }

                    }

                });


            });
        </script>

        <footer>
            <div class="container">
                <ul>
                    <li><a href="https://www.benchmark.bg/academy/%D0%B7%D0%B0-%D0%BD%D0%B0%D1%81/" target="_blank">За нас</a></li>
                    <li>|</li>
                    <li><a href="http://benchmark.bg" target="_blank">Към основния сайт</a></li>
                    <li>|</li>
                    <li><a href="https://www.benchmark.bg/academy/%D0%BA%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%D0%B8/" target="_blank">Контакти</a></li>
                </ul>
            </div>
        </footer>


    </body>
</html>